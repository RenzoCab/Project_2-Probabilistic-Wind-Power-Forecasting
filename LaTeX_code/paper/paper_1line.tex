...
\affil[1,3]{CEMSE Division, King Abdullah University of Science and Technology (KAUST), Saudi Arabia}
\affil[3]{Alexander von Humboldt Professor, RWTH Aachen University, Germany}
...
{\color{red} Analysis of the autocorrelation, removing days to achieve independence.} 
...
\begin{figure}
  ...
  \caption{Two samples from the Uruguayan of 2018. Each sample compromises of two 72-hour paths. In yellow is an hourly wind power production forecast. In blue is the actual wind power production recorded in 10 minute intervals. {\color{red} New figures with 2019 Uruguayan dataset.}}
...
\begin{figure}
  ...
  \caption{We see that forecast errors exhibit skewness near the boundries (i.e. low and high power production regimes.). Low power is when produced power is less than $0.3$, mid power is when it is between $0.3$ and $0.6$ and high power when is larger than $0.6$.   {\color{red} New figures with 2019 Uruguayan dataset.}}
  \label{fig:skew_data}
\end{figure}
...
\begin{figure}
  ...
  \caption{We observe that skewness has been greatly reduced after the Lamperti transformation. This motivates us to use a Gaussian transition density as a proxy density.  {\color{red} New figures with 2019 Uruguayan dataset.}}
...
\begin{figure}
  ...
  \caption{Transitions after the Lamperti transform.}
  \label{fig:skew_after_lamperti}
\end{figure}
%---END SECTION 1---
...
{\color{red} The expression for $\theta_t$ is important: $\theta_t$ depens on $t$ through $\dot{p_t}$ and $p_t$. Besides it is parsimonious because only one unkonwn parameter appear in \ref{theta_t}. } 
...
\begin{equation}
\mathcal{L}(\bm{\theta}; V^{M,N}) = \prod\limits_{j=1}^M \prod\limits_{i=1}^N \rho ( {V_{j, t_i}| V_{j, t_{i-1}}} ; p_{[t_{i-1}, t_{i}]},  \bm{\theta} )  \, \rho_0 (V_{j, t_0}) \,.
\label{likelihood}
\end{equation}
...
\begin{figure}
  ...
  \caption{We show self-convergence of our algorithm applied to model $2$. We conlcude that the rate  matches that of of Monte Carlo. Data is from Uruguay pertaining to  year 2018.{\color{red} Figure with data 2019} }
\end{figure}

    \begin{figure}[h]
      \includegraphics[width=70mm,scale=1]{simulated/24hr/1099.pdf}
      \includegraphics[width=70mm,scale=1]{simulated/24hr/1178.pdf}
      \caption{We simulate following model 2  five possible future wind power production paths using the obtained optimal parameters $(\theta_0, \alpha )=(12,0.3)$. Forecast is from Uruguay pertaining to  year 2018. {\color{red} Figure with data 2019}}
    \end{figure}
    ...
    \begin{figure}[h]
      ...
      \includegraphics[width=70mm,scale=1]{confidence_intervals/24hr/820.pdf}
       \caption{We obtain confidence intervals following model 2  for future wind power production using the obtained optimal parameters $(\theta_0, \alpha )=(12,0.3)$. Actual production plotted in retrospect. Forecast and data is from Uruguay pertaining to  year 2018. {\color{red} Figure with data 2019}}
    \end{figure}
%---END SECTION 7---
