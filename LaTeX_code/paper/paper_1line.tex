...
We can see in Figure (\ref{fig:skew_after_lamperti}) the effects of the Lamperti transform on the forecast error data. 
The Lamperti transform has greatly reduced the forecast error skewness, ensuring that the process stays in the range $[0,1]$. Therefore, in this case, the transition densities of the process Z can be approximated through Gaussian densities.
%---END SECTION 3---
...
The It\^{o} process $V_t$ defined by the SDE (\ref{VtSDE}) is Markovian, then the likelihood function can be written as a product of transition densities.  Consider the set of $M$ paths with $N$ observations each, $ V^{M,N}=\{ V_{t_1^{M,N}} , V_{t_2^{M,N}} ,\ldots , V_{t_N^{M,N}} \}$, observed in $\Delta_N$ intervals.
