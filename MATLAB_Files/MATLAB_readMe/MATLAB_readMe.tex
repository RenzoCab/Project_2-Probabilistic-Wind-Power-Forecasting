...
\alert{Check if still correct for the new model!!!}\\
...
\begin{equation*}
\theta(t) = \max\left(\theta_0,\frac{\theta_0\alpha+2|\dot{p}^\epsilon(t)|}{2\min(p^\epsilon(t),1-p^\epsilon(t))}\right),\quad\theta_0>0.
\end{equation*}
\quad\\
\quad\\
\begin{center}
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.32\textwidth]{../Results/likelihood/Log-Likelihood.eps}
\includegraphics[width=0.32\textwidth]{../Results/likelihood/Log-Likelihood_refined.eps}
\includegraphics[width=0.32\textwidth]{../Results/likelihood/Log-Likelihood_more_refined.eps}
...
\quad\\
\quad\\
They have the same partial derivatives. The main difference appears in the resulting SDE for $Z_t$. In the {\color{blue} first case}, we have a $\sin(\sqrt{2\alpha\theta_0}Z_t)$ in the drift, while in the \alert{second case}, we have $\cos(\sqrt{2\alpha\theta_0}Z_t)$.\\
\quad\\
...
\setbeamercolor{background canvas}{bg=white!10}
\begin{frame}\frametitle{Lamperti data:}

We are following the {\color{blue}first candidate}. Then, we caa
...
The Lamperti SDE is:
\begin{align*}
\dif Z_t= \underbrace{\left[  \frac{(\alpha \theta_0 - \theta_t) {\color{blue}\sin(\sqrt{2 \alpha \theta_0 } Z_t) }- \theta_t (1 - 2 p_t) + 2  \dot{p}_t }{\sqrt{2 \alpha \theta_0} \cos{(\sqrt{2 \alpha \theta_0} Z_t)}}  \right]}_{\coloneqq b(Z_t)} \dif t + \dif W_t . 
\end{align*}
