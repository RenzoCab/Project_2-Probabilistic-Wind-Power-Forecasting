function [batch_theta] = batch_with_theta_L(batch, alpha, theta_0) % 29/03/2020 19:43
    ...
    batch(5,1)   = lamperti_transform(theta_0,alpha,batch(3,1),batch(1,1));
    batch(5,end) = lamperti_transform(theta_0,alpha,batch(3,end),batch(1,end));
    
    for i = 2:2:length(batch(1,:))-1
        batch(4,i:i+1) = theta_t(theta_0, alpha, batch(1,i), batch(2,i));
        batch(5,i:i+1) = lamperti_transform(theta_0, alpha, batch(3,i), batch(1,i));
    end
