...
max_Z
norm = cm.colors.Normalize(vmax= abs(Z/max_Z).max(), vmin=abs(Z/max_Z ).min())
cmap = cm.RdGy
contours=plt.contourf(X, Y, Z/max_Z,100, cmap=cmap)
plt.colorbar()
plt.title('Beta Distribution Log-likelihood \n (' + str(M) +' samples, $\\Delta N= $'+'{:.1e}'.format(dN)+')'); #,fontsize=24
plt.xlabel('$\\theta$');
plt.ylabel('$\\alpha$');
...
plt.clf()
contours=plt.contourf(X, Y, Z_grad_norm/max_Z_grad_norm, 100,norm=norm, cmap=cmap)
