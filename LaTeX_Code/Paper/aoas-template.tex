...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{plots/wind_and_solar/plot_over_years_IRENA.eps}
\caption{Worldwide installed wind and solar power 2010--2019 \citep{irena20}. We recall the importance of accurate forecasts to use green energies optimally.}
\end{figure}
...
Reliable wind power generation forecasting is crucial for the following applications (see, for example, \cite{gieb}, \cite{chang}, \cite{zhbo}):
\begin{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.485\textwidth]{plots/four_days/1.eps}\quad
\includegraphics[width=0.485\textwidth]{plots/four_days/3.eps}\\
%\quad\\
%\includegraphics[width=0.45\textwidth]{plots/four_days/437.eps}
%\includegraphics[width=0.45\textwidth]{plots/four_days/477.eps}
\caption{Two 24-hour segments with the normalized wind power real production in Uruguay (blue line) recorded every ten minutes, and the hourly wind power production forecasted by provider A (black line).}
...
\begin{figure}[H]
\centering
  \includegraphics[width=0.485\textwidth]{plots/1.eps}\quad\includegraphics[width=0.485\textwidth]{plots/bands/1.eps}
...
The proof of this theoretical statement is presented in the Appendix. \\
...
\end{equation*}
with the discrete sum $\sum_{0< t_{j, i-1} \leq t}\left(V_{t_{j, i}} - V_{t_{j, i-1}}\right)^2$.\smallskip
...
\toprule
 ...
 \bottomrule
\end{tabular}
\caption{Coordinates of the initial guess point and of the optimal points represented in Figure \ref{fig:neg-LL}.}
...
This research was partially supported by the KAUST Office of Sponsored Research (OSR) under Award number URF/1/2584 -- 01 -- 01 in the KAUST Competitive Research Grants Program Round 8, the Alexander von Humboldt Foundation, the chair Risques Financiers, Fondation du Risque, and the Laboratory of Excellence MME-DII Grant no. ANR11-LBX--0023--01 (\url{http://labex-mme-dii.u-cergy.fr/}). We thank UTE (\url{https://portal.ute.com.uy/}) for providing the data used in this research.
...
\sname{Supplement}
\stitle{DATA AND SIMULATIONS}
\slink[doi]{COMPLETED BY THE TYPESETTER}
\sdatatype{.pdf}
\sdescription{Supplementary Material contains the entire dataset with normalized wind-power production, the simulation-based results, and the complete set of empirical pointwise confidence bands for wind power production -- all corresponding to the forecast provider A.}
\end{supplement}
