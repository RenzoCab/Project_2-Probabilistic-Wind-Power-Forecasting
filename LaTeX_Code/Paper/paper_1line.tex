...
\begin{equation}
\mathcal{L}\left(\bm{\theta}; V^{M,N +1}\right) = \prod\limits_{j=1}^M \left\{ \prod\limits_{i=1}^N \rho ( {V_{j, i}| V_{j, i-1}} ; p_{[t_{j,  i-1}, t_{j , i} ]},  \bm{\theta} )    \right\},
\label{likelihood}
\end{equation}
...
\label{Eq-4}
\end{equation}
\red{We approximate $\theta_0$ by Equation (\ref{Eq-4}) setting $\theta_0^*=c^*$.}
...
\toprule
 ...
 \midrule
 Initial guess & 1.54 & 0.070 & 0.111 & 1.96 & 0.053 & 0.104 \\
...
\end{table}
Observe that all the local (possibly global) minimizers are located over the curves $\theta_0\alpha=0.097$ and $\theta_0\alpha=0.089$ for the training and test sets, respectively.
