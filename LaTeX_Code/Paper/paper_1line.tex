...
For any $j=1, \ldots, M$, we extrapolate linearly the truncated prediction function to get the value for $p_{j,-\delta}$, and set $v(t_{j,-\delta})=0$. In this way, we consider the initial transition $(V_{j, 0}|v_{j,-\delta};\bm{\theta},\delta)$. 
We assume that this initial transition has a Beta distribution and apply the same moment matching method as for the rest of the transitions described above. Given an initial guess for $\bm{\theta}$, we obtain the initial estimation for $\delta$ solving the following problem
\begin{equation}
...
\quad\\
As remarked, we can extend the statistical model (\ref{likelihood}) to include the extra parameter $\delta$. The complete likelihood, $\mathcal{L}_c$, is given by
\begin{equation}
...
\end{equation}
As we may provide initial guesses for $\bm{\theta}$ and $\delta$, we have a starting point for the maximization of (\ref{eq:complete_LH}).
