...
Now, looking at the normalized wind power generation forecast process $X$, modeled as solution to the It\^{o} stochastic differential equation (\ref{eq:main}) with the drift specified in (\ref{drift:meanrev-derivtrack}), it is straightforward to check that $\mathbb{E} \left[X_t\right] = p_t$, given $X_0 = p_0$. The application of  It\^{o}'s lemma on $g(X_t, t) = X_t e^{\int_{0}^{t} \theta_s ds}$, leads to 
\begin{equation*}
...
\end{itemize} 
{\color{red} Observe that a mean reverting model without derivative tracking exhibits consistent lags, as it is shown in Figure (\ref{fig:derivative_tracking}).} See Section \ref{Section_7} for comparisons.
...
& = \frac{1}{\sqrt{2 \alpha \theta_0 }} \arcsin (2(V_t + p_t) -1)
\end{align}
...
\end{align}
After replacing $V_t = \frac{\sin(\sqrt{2 \alpha \theta_0} Z_t) + 1}{2} - p_t$ in (\ref{eq:stindepSDE}), we obtain that the  process $Z$ satisfies the SDE
\begin{align}
dZ_t & = \Bigg[ \frac{2 \dot{p}_t}{\sqrt{2 \alpha \theta_0} \cos{(\sqrt{2 \alpha \theta_0} Z_t)}} + \frac{- \theta_t (\sin(\sqrt{2 \alpha \theta_0} Z_t) + 1 - 2 p_t)}{\sqrt{2 \alpha \theta_0} \cos{(\sqrt{2 \alpha \theta_0} Z_t)}} \nonumber \\
&  + \frac{\sqrt{2 \alpha \theta_0} \sin{(\sqrt{2 \alpha \theta_0} Z_t)}}{2 \cos{(\sqrt{2 \alpha \theta_0} Z_t)}} \Bigg] \,dt + dW_t \nonumber \\
&  = \left[  \frac{(\alpha \theta_0 - \theta_t) \sin(\sqrt{2 \alpha \theta_0 } Z_t) - \theta_t (1 - 2 p_t) + 2  \dot{p}_t }{\sqrt{2 \alpha \theta_0} \cos{(\sqrt{2 \alpha \theta_0} Z_t)}}  \right] \,dt + dW_t \,. 
\end{align}
