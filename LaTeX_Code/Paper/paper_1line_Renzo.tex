...
To guarantee the good behave for our optimization algorithm, we aim to start the optimization as close as we can from the optimal parameters. We use \textbf{Lease Square Minimization} and \textbf{Quadratic Variation} to find an initial guess from the data.

\begin{itemize}

\item \textbf{Least Square Minimization:} We consider the transition $\Delta V_i=V_{i+1}-V_i$ with $\Delta t=t_{i+1}-t_i$. $(V_{i+1}|V_i)$ is a random variable which conditional mean can be approximated by the solution of the system
\begin{equation*}
\begin{cases}
...
If we assume that $\theta_t^\epsilon={\color{red}c}\in\R^+$ for all $t\in[t_i,t_{i+1}]$, then $\E\left[V(t_{i+1})\right]=V_ie^{-{\color{red}c}\Delta t}$. If we have a total of $n$ transitions, we can write the regression problem for the conditional mean with $L^2$ loss function as
\begin{equation}
{\color{red}c}^*\approx\arg\min_{{\color{red}c}\geq0}\left[\sum_{i=1}^n\left(V_{i+1}-\E\left[V(t_{i+1})\right]\right)^2\right]=\arg\min_{{\color{red}c}\geq0}\left[\sum_{i=1}^n\left(V_{i+1}-V_ie^{-{\color{red}c}\Delta t}\right)^2\right].
\label{Eq-1}
\end{equation}
\begin{equation*}
\begin{split}
\frac{\partial f}{\partial{\color{red}c}}&=\sum_{i=1}^n2(-V_i)(-\Delta t)(V_{i+1}-V_i(1-\theta_0\Delta t))\\
&=\sum_{i=1}^n2V_i\Delta t(V_{i+1}-V_i(1-{\color{red}c}\Delta t))\\
&=\sum_{i=1}^n2V_{i+1}V_i\Delta t-2V_i^2\Delta t+2V_i^2\Delta t^2{\color{red}c}.
\end{split}
\end{equation*}
Then, ${\color{red}c}^{**}$ satisfies
\begin{equation}
{\color{red}c}^{**}\approx\frac{\sum_{i=1}^nV_i(V_i-V_{i+1})}{\Delta t\cdot\sum_{i=1}^n(V_i)^2}.
\label{Eq-4}
\end{equation}
Notice that ${\color{red}c}^{**}$ has dimension $\mathbf{time}^{-1}$.
...
\quad\\
Recall from slide ({\color{blue}\ref{S1}}) that, to use the LSM estimation, we assume $\theta_t^\epsilon={\color{red}c}\in\R^+$. Recall also our definition for $\theta_t^\epsilon$ (equation ({\color{blue}\ref{Eq-11}})):
\begin{equation*}
...
\item We create $\tilde{\mathbf{J}}$ and use the LSM estimation to find $k$. If $k<\theta_0^*$, then the assumption $\theta_t^\epsilon={\color{red}c}\in\R^+$ is wrong and we reduce the value of $\epsilon$, i.e., $\epsilon\gets\epsilon*0.999$. If $k\geq\theta_0^*$, we load $\epsilon\gets\frac{\theta_0^*\alpha^*}{k}$ (we allow a maximum relative change of 1\%). We repeat this step *100 times*.
