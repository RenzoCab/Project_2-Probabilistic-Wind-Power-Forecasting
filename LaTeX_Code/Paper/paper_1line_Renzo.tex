...
\item \textbf{Least square minimization:} We consider the observed data $v^{M,N+1}$ with length between observations $\Delta$, where $i\in\{0,\dots,N\}$ and $j\in\{1,\dots,M\}$. $(V_{j,i+1}|v_{j,i})$ is a random variable which conditional mean can be approximated for any $t\in [t_{j,i-1}, t_{j, i})$ by the solution of the system
\begin{equation*}
\begin{cases}
\dif \E\left[V\right]&=-\theta_t\E\left[V\right]\dif t\\
\E\left[V\right](t_{j,i})&=v_{j,i},
\end{cases}
\end{equation*}
evaluated in $t_{j,i+1}$, i.e., $\E\left[V\right](t_{j,i+1})$. Then, the random variable $(V_{j,i+1}-\E\left[V\right](t_{j,i+1}))$ has zero mean. If we assume that $\theta_t=c\in\R^+$ for all $t\in[t_{j,i},t_{j,i+1}]$, then $\E\left[V(t_{j,i+1})\right]=v_{j,i}e^{-c\Delta}$. If we have a total of $M\times N$ transitions, we can write the regression problem for the conditional mean with $L^2$ loss function as
\begin{equation}
\begin{split}
c^*&\approx\arg\min_{c\ \geq\ 0}\left[\sum_{j=1}^M\sum_{i=0}^{N-1}\left(v_{j,i+1}-\E\left[V\right](t_{j,i+1})\right)^2\right]\\
...
\begin{equation}
\bm{\theta}^*=\arg\max_{\bm{\theta}}\tilde{\ell}_Z\left(\bm{\theta};\{h( v_{j,i};\bm{\theta}^*)\}_{j=1,i=1}^{M,N}\right).
\label{FP}
\end{equation}
...
\begin{equation*}
\bm{\theta}^*-\arg\max_{\bm{\theta}}\tilde{\ell}_Z\left(\bm{\theta};\{h( v_{j,i};\bm{\theta}^*)\}_{j=1,i=1}^{M,N}\right)=0.
\end{equation*}
Given $\bm{\theta}^*=(\theta_0^*,\alpha^*)$, we call $\bm{\theta}^{**}=(\theta_0^{**},\alpha^{**})$ to the solution of
\begin{equation*}
\arg\max_{\bm{\theta}}\tilde{\ell}_Z\left(\bm{\theta};\{h( v_{j,i};\bm{\theta}^*)\}_{j=1,i=1}^{M,N}\right).
\end{equation*}
...
\label{fig:neg-LL}
\end{figure}
On Figure (\ref{fig:neg-LL}), we can see the level sets for the negative log-likelihood. The numerical values of each important point can be seen in Table (\ref{tab:optimal_values}). We set $\bm{\theta}^V=(\theta_0^V,\alpha^V)=(1.93,0.050)$, as it is where the negative log-likelihood reaches its minimum value.
\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
...
We obtain $(\theta_0^Z,\alpha^Z)=(1.87,0.043)$ as a minimizer for the relative error function (\ref{eq:rel_error}).\\
\quad\\
